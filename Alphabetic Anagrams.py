...
def listPosition(word):
    ...
    while len(word):
        ...
        possible_orders = fact(len(word))
        
        for char in unique_chars:
            possible_orders = possible_orders / fact(word.count(char))

        for char in unique_chars:
            if char < first_char: 
                count += trunc(possible_orders/len(word)) * word.count(char)
        word = word[1:]
    return trunc(count) + 1